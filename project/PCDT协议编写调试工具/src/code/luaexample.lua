...
function Ctrl_AODO_Send_cmd(addr, ctrlnumber, val )	 -- 控制命令部分,自行实现
	...
	local DataBuf={}
	--私有协议log.info("NodeID= "..NodeID)
	--[[
		例：
		if( NodeID==204 ) then	-- 开整流器测试
			Length = 2
			DataBuf[0] = 0x11
			DataBuf[1] = 0x00
			return YDT1363_Send_Ctrl_Cmd(addr,Length,DataBuf,2) 
		end
	--]]
...
-- 接收区区分
	...
	--[[
		例：
		if( scanIdx==1) then	-- 交流系统模拟量量化数据(40_42_00)	
		if( (CID1==0x40) and (Length>=16) ) then
			DataOffset = 2
			GroupType[2] = PCM_DEV_Get_U8(GroupType[2], recvBuf, recvBufLen, DataOffset, RecvData)  -- 交流路数
			DataOffset = DataOffset + 1
			
			local CycleNo = RecvData[1]
			for i=1,CycleNo do
				if( i==1 ) then
					GroupType[3] = PCM_DEV_Get_U16(GroupType[3], recvBuf, recvBufLen, DataOffset, RecvData) -- 1#输入线/相电压AB/A
					DataOffset = DataOffset + 2
					for n=1,p do
						if( n<=6 ) then
							GroupType[6+n] = PCM_DEV_Get_U16(GroupType[6+n], recvBuf, recvBufLen, DataOffset, RecvData) 
						end
						DataOffset = DataOffset + 2
					end
				elseif( i==2 ) then
					GroupType[13] = PCM_DEV_Get_U16(GroupType[13], recvBuf, recvBufLen, DataOffset, RecvData) -- 2#输入线/相电压AB/A
					DataOffset = DataOffset + 2
					for n=1,p do
						if( n<=6 ) then
							GroupType[16+n] = PCM_DEV_Get_U16(GroupType[16+n], recvBuf, recvBufLen, DataOffset, RecvData) 
						end
						DataOffset = DataOffset + 2
					end
				else
					DataOffset = DataOffset + 8
					local p = PCM_DEV_Get_U8(nil, recvBuf, recvBufLen, DataOffset, RecvData) -- 用户自定义遥测数量p
					DataOffset = DataOffset + 1
					for n=1,p do
						DataOffset = DataOffset + 2
					end
			...
			GroupType[23] = PCM_DEV_Get_U16(GroupType[23], recvBuf, recvBufLen, DataOffset, RecvData,"/10") -- 交流输出电流A
			DataOffset = DataOffset + 2
		end
	end
	--]]
