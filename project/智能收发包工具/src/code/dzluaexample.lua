...
function parse_AODO(ack_package)		-- 无需改动	
    data, errmsg = gmoe.depack(ack_package) -- check the RTN(cid2) == '00'
    if (data == nil) then
...
function scan (step, addr)				-- 无需改动
	...
	if( step <= number ) then
		...
		return 0,  gmoe.pack(ver, addr, CID1, CID2, DataParam)
	end
	return 0,""
end
function parse (step, package)			-- 无需改动
    ...
    data, errmsg = gmoe.depack(package)
    if (data == nil) then
...
function YDT1363_Send_Ctrl_Cmd(addr, Paramlen, buf, step)	--输入参数:地址,参数长度,参数xx,命令序号   返回值  :发包命令
	...
	return gmoe.pack(ver, addr, CID1, CID2, DataParam)
end
-- Lua模版结束
